# This file is modified from the function CarliniWagnerL2Attack implemented
# by AdverTorch: https://github.com/BorealisAI/advertorch
#
...
class FindNarrowness(Attack, LabelMixin):
    """
    modified from the function CarliniWagnerL2Attack
    ...
    def __init__(self, predict, num_classes, confidence=0,
        ...
        super(FindNarrowness, self).__init__(
    ...
    def _forward_and_update_delta(
            self, optimizer, x, delta, y_onehot, loss_coeffs):

        optimizer.zero_grad()
        adv1 = torch.clamp(delta + x, self.clip_min, self.clip_max)
        adv2 = torch.clamp(delta - x, self.clip_min, self.clip_max)

        output1 = self.predict(adv1)
        output2 = self.predict(adv2)

        l2distsq = delta.view(delta.shape[0], -1).sum(dim=1)
        loss1 = self._cwl2_loss(output1, y_onehot) + self._cwl2_loss(output2, y_onehot)
        loss1 = torch.sum(loss_coeffs * loss1)
        loss2 = l2distsq.sum()
        loss = loss1 + loss2
        loss.backward()
        optimizer.step()

        return loss.item(), l2distsq.data, (output1.data, output2.data), (adv1.data, adv2.data)
    ...
    def _update_if_smaller_dist_succeed(
            self, adv_img1, adv_img2, labs, output1, output2, l2distsq, batch_size,
            cur_l2distsqs, cur_labels1, cur_labels2,
            final_l2distsqs, final_labels1, final_labels2, final_advs1, final_advs2):

        target_label = labs
        output_logits1 = output1
        _, output_label1 = torch.max(output_logits1, 1)
        output_logits2 = output2
        _, output_label2 = torch.max(output_logits2, 1)

        mask = (l2distsq < cur_l2distsqs) & self._is_successful(
            output_logits1, target_label, True) & self._is_successful(
            output_logits2, target_label, True)

        cur_l2distsqs[mask] = l2distsq[mask]  # redundant
        cur_labels1[mask] = output_label1[mask]
        cur_labels2[mask] = output_label2[mask]

        mask = (l2distsq < final_l2distsqs) & self._is_successful(
            output_logits1, target_label, True) & self._is_successful(
            output_logits2, target_label, True)
        final_l2distsqs[mask] = l2distsq[mask]
        final_labels1[mask] = output_label1[mask]
        final_labels2[mask] = output_label2[mask]
        final_advs1[mask] = adv_img1[mask]
        final_advs2[mask] = adv_img2[mask]

    def _update_loss_coeffs(
            self, labs, cur_labels1, cur_labels2, batch_size, loss_coeffs,
            coeff_upper_bound, coeff_lower_bound):

        # TODO: remove for loop, not significant, since only called during each
        # binary search step
        for ii in range(batch_size):
            cur_labels1[ii] = int(cur_labels1[ii])
            cur_labels2[ii] = int(cur_labels2[ii])
            if self._is_successful(cur_labels1[ii], labs[ii], False) and self._is_successful(cur_labels2[ii], labs[ii], False):
    ...
    def perturb(self, x, y=None):
        ...
        final_labels1 = [INVALID_LABEL] * batch_size
        final_labels2 = [INVALID_LABEL] * batch_size
        final_advs1 = x.detach().clone()
        final_advs2 = x.detach().clone()
        y_onehot = to_one_hot(y, self.num_classes).float()

        final_l2distsqs = torch.FloatTensor(final_l2distsqs).to(x.device)
        final_labels1 = torch.LongTensor(final_labels1).to(x.device)
        final_labels2 = torch.LongTensor(final_labels2).to(x.device)
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            cur_labels1 = [INVALID_LABEL] * batch_size
            cur_labels2 = [INVALID_LABEL] * batch_size
            cur_l2distsqs = torch.FloatTensor(cur_l2distsqs).to(x.device)
            cur_labels1 = torch.LongTensor(cur_labels1).to(x.device)
            cur_labels2 = torch.LongTensor(cur_labels2).to(x.device)
            prevloss = PREV_LOSS_INIT

            if (self.repeat and outer_step == (self.binary_search_steps - 1)):
                loss_coeffs = coeff_upper_bound
            for ii in range(self.max_iterations):
                loss, l2distsq, (output1, output2), (adv_img1, adv_img2) = \
                    self._forward_and_update_delta(
                        optimizer, x, delta, y_onehot, loss_coeffs)
                if self.abort_early:
                    if ii % (self.max_iterations // NUM_CHECKS or 1) == 0:
                        if loss > prevloss * ONE_MINUS_EPS:
                            break
                        prevloss = loss
                print(output1.argmax(-1), y, output2.argmax(-1))
                self._update_if_smaller_dist_succeed(
                    adv_img1, adv_img2, y, output1, output2, l2distsq, batch_size,
                    cur_l2distsqs, cur_labels1, cur_labels2,
                    final_l2distsqs, final_labels1, final_labels2, final_advs1, final_advs2)

            self._update_loss_coeffs(
                y, cur_labels1, cur_labels2, batch_size,
                loss_coeffs, coeff_upper_bound, coeff_lower_bound)

        return final_advs1, final_advs2, delta.detach().clone()
