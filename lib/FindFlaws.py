...
class FindFlaws(Attack, LabelMixin):
    """
    ...
    def _forward_and_update_delta(
        ...
        # one adv
        adv1 = tanh_rescale(delta + x_atanh, self.clip_min, self.clip_max)
        transimgs_rescale1 = tanh_rescale(x_atanh, self.clip_min, self.clip_max)
        output1 = self.predict(adv1)
        l2distsq1 = calc_l2distsq(adv1, transimgs_rescale1)
        loss1 = self._loss_fn(output1, y_onehot, l2distsq1, loss_coeffs)
        loss1.backward()

        # symmetric adv
        adv2 = tanh_rescale(-delta + x_atanh, self.clip_min, self.clip_max)
        transimgs_rescale2 = tanh_rescale(x_atanh, self.clip_min, self.clip_max)
        output2 = self.predict(adv2)
        l2distsq2 = calc_l2distsq(adv2, transimgs_rescale2)
        loss2 = self._loss_fn(output2, y_onehot, l2distsq2, loss_coeffs)
        loss2.backward()

        optimizer.step()

        return loss1.item(), l2distsq1.data, output1.data, adv1.data,\
               loss2.item(), l2distsq2.data, output2.data, adv2.data
    ...
    def perturb(self, x, y=None):
        ...
        final_l2distsqs1 = [CARLINI_L2DIST_UPPER] * batch_size
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            cur_l2distsqs1 = [CARLINI_L2DIST_UPPER] * batch_size
            cur_labels1 = [INVALID_LABEL] * batch_size
            cur_l2distsqs1 = torch.FloatTensor(cur_l2distsqs1).to(x.device)
            cur_labels1 = torch.LongTensor(cur_labels1).to(x.device)

            cur_l2distsqs2 = [CARLINI_L2DIST_UPPER] * batch_size
            cur_labels2 = [INVALID_LABEL] * batch_size
            cur_l2distsqs2 = torch.FloatTensor(cur_l2distsqs2).to(x.device)
            cur_labels2 = torch.LongTensor(cur_labels2).to(x.device)

            prevloss1 = PREV_LOSS_INIT
            prevloss2 = PREV_LOSS_INIT

            if (self.repeat and outer_step == (self.binary_search_steps - 1)):
                loss_coeffs = coeff_upper_bound
            for ii in range(self.max_iterations):
                loss1, l2distsq1, output1, adv_img1,\
                loss2, l2distsq2, output2, adv_img2 = \
                    self._forward_and_update_delta(
                        optimizer, x_atanh, delta, y_onehot, loss_coeffs)
                if self.abort_early:
                    if ii % (self.max_iterations // NUM_CHECKS or 1) == 0:
                        if (loss1 > prevloss1 * ONE_MINUS_EPS) and \
                            (loss2 > prevloss2 * ONE_MINUS_EPS):
                            break
                        prevloss1 = loss1
                        prevloss2 = loss2
